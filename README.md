...
Веб-приложение для создания PDF-прописей для обучения письму.

## Функции

- Создание PDF-прописей с различными вариантами заполнения
- Размножение первой буквы в строке
- Размножение одной строки на всю страницу
- Различные форматы разметки (линии, линии с наклоном, клетки)
- Выбор ориентации страницы и стиля шрифта
- Поддержка как клиентской, так и серверной генерации PDF
...
1. Клонировать репозиторий:
   ```
   git clone https://github.com/username/propisi.git
   cd propisi
   ```
...
3. Запустить FastAPI бэкенд:
   ```
   uvicorn api.generate-pdf:app --reload
   ```

4. Открыть в браузере `http://localhost:8000/static/pdf-generator.html`

### Деплой на Vercel

1. Создать проект на Vercel
2. Связать с репозиторием GitHub
3. Настроить переменные окружения (если необходимо)
4. Vercel автоматически распознает `vercel.json` и настроит деплой

## Устранение проблем

Если при генерации PDF возникают ошибки 500 на сервере, попробуйте:

1. Переключиться на клиентский режим генерации PDF
2. Проверить логи сервера через панель Vercel
3. Убедиться, что размер данных не превышает лимиты Vercel Functions (10 секунд выполнения)

## Техническая информация

- Frontend: HTML, CSS, JavaScript с библиотекой pdfmake
- Backend: Python с FastAPI и библиотекой fpdf2
